...
import { badgeStyle, resolveColor } from "@/lib/warehouse-colors";
    ...
    <div className="flex flex-wrap gap-1">
      {stocks.map((s) => (
        <span
          ...
          className={
            // Нулевой остаток всегда серый — это состояние товара, а не склад.
            (s.qty > 0 ? "" : "border-line bg-gray-50 text-muted ") +
            "badge border" +
            (tooltip ? " cursor-help" : "")
          }
          // Цвет закреплён за складом и правится в админке: три одинаково
          // зелёные плашки в строке было не различить с одного взгляда.
          // Стиль строится из HEX — палитра не ограничена набором классов.
          style={
            s.qty > 0
              ? badgeStyle(resolveColor(s.color, s.warehouse))
              : undefined
          }
        >
