...
export type OneCOrderPayload = {
  ...
  products: {
    ...
    qty: number;
    price: number;
    // Склад строки. Он совпадает со складом документа (документ всегда
    // односкладский), но продублирован в каждой строке, чтобы 1С могла
    // заполнять колонку «Со склада» прямо из строки выгрузки.
    warehouse?: string | null;
  }[];
};
